...
namespace Datadog.Profiler.IntegrationTests.Helpers
{
    internal readonly struct StackFrame : IEquatable<StackFrame>
    {
        ...
        public StackFrame(string rawStackFrame, string filename, long line)
        {
            ...
            if (!match.Success)
            {
                // Pyroscope uses "namespace!type.method" frame names instead of
                // Datadog's structured "|lm:... |ns:..." representation.
                var namespaceSeparator = rawStackFrame.IndexOf('!');
                var member = namespaceSeparator >= 0 ? rawStackFrame[(namespaceSeparator + 1)..] : rawStackFrame;
                var methodSeparator = member.LastIndexOf('.');

                Module = namespaceSeparator >= 0 ? rawStackFrame[..namespaceSeparator] : string.Empty;
                Namespace = Module;
                Type = methodSeparator >= 0 ? member[..methodSeparator] : member;
                TypeAdornment = string.Empty;
                Function = methodSeparator >= 0 ? member[(methodSeparator + 1)..] : member;
                FunctionAdornment = string.Empty;

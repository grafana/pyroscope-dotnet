    ...
    ClassID* genericParameters)
{
    ...
    for (ULONG32 i = 0; i < genericParametersCount; i++)
    {
        ...
        {
            // Fork convention: '!' separates namespace from type (see FormatFrame),
            // not '.' as upstream uses. Preserve it here. Adopt upstream #8961's fix
            // for namespaceless types: a type declared outside of any namespace has
            // no namespace, so don't emit a leading separator.
            if (!ns.empty())
            {
                builder << ns << "!";
            }
